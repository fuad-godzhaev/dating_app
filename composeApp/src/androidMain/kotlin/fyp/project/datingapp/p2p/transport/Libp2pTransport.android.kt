...
import fyp.project.datingapp.golibp2p.Golibp2p
import fyp.project.datingapp.golibp2p.GossipCallback
import fyp.project.datingapp.golibp2p.ProviderCallback
import fyp.project.datingapp.golibp2p.StreamCallback
...
import fyp.project.datingapp.golibp2p.Host as GoHost
import fyp.project.datingapp.golibp2p.Stream as GoStream
 ...
 */
actual class Libp2pTransport actual constructor(private val config: Libp2pConfig) {
    ...
    actual suspend fun connect(multiaddr: String) {
        withContext(Dispatchers.IO) { requireHost().connect(multiaddr) }
    }

    actual val peerId: String get() = requireHost().peerID()

    actual val listenAddrs: List<String>
        get() = requireHost().listenAddrs().split('\n').filter { it.isNotBlank() }

    // ---- DHT ----

    actual suspend fun dhtPutValue(key: ByteArray, value: ByteArray) {
        withContext(Dispatchers.IO) { requireHost().dhtPut(key, value) }
    }

    actual suspend fun dhtGetValues(key: ByteArray, maxResults: Int): List<ByteArray> =
        withContext(Dispatchers.IO) {
            val list = requireHost().dhtGet(key, maxResults.toLong())
            (0 until list.len()).map { list.get(it) }
        }

    actual suspend fun dhtProvide(key: ByteArray) {
        withContext(Dispatchers.IO) { requireHost().dhtProvide(key) }
    }

    actual fun dhtFindProviders(key: ByteArray): Flow<String> = callbackFlow {
    ...
    actual fun gossipSubscribe(topic: String): Flow<GossipMessage> = callbackFlow {
    ...
    actual suspend fun gossipUnsubscribe(topic: String) {
        withContext(Dispatchers.IO) { requireHost().gossipUnsubscribe(topic) }
    }

    actual suspend fun gossipPublish(topic: String, payload: ByteArray) {
        withContext(Dispatchers.IO) { requireHost().gossipPublish(topic, payload) }
    }

    // ---- Streams ----

    actual fun registerStreamHandler(protocolId: String, handler: StreamHandler) {
    ...
    actual suspend fun openStream(remotePeerId: String, protocolId: String): Libp2pStream =
        withContext(Dispatchers.IO) {
