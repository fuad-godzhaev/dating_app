...
import com.aura.domain.auth.P256
...
import com.aura.p2p.discovery.DiscoveryFilters
 ...
 * end: host boot (B3), a /aura/ping round-trip, a DHT put/get, and a
...
object P2pSmoke {
    ...
    private const val PING = "/aura/ping/1.0.0"
    private const val TOPIC = "/aura/smoke"
     ...
     * (/aura/profile/1.0.0), verifies the owner signature, decodes, and logs
    ...
    suspend fun runProfileFetch(
        ...
        peerDirectory: com.aura.p2p.discovery.PeerDirectory,
        geohash: String,
    ) {
        val scope = CoroutineScope(SupervisorJob() + Dispatchers.IO)
        var nsd: NsdDiscovery? = null
            ...
            scope.launch {
                discovery.candidates().collect { rec ->
                    if (fetched.add(rec.did)) {
                        ...
                        scope.launch {
                            var profile: com.aura.records.UserProfile? = null
                            repeat(10) { attempt ->
                                if (profile == null) {
