...
class DefaultSignInComponent(
    ...
    init {
        // Load the display name (for "Welcome back, {name}") + own photo for the avatar (best-effort).
        scope.launch {
            val profile = runCatching { repositoryManager.getMyProfile() }.getOrNull()
            val ref = profile?.photos?.firstOrNull()?.ref
            val path = ref?.let { runCatching { repositoryManager.blobFilePath(it) }.getOrNull() }
            _state.value = _state.value.copy(displayName = profile?.displayName, photoPath = path)
        }
    }
