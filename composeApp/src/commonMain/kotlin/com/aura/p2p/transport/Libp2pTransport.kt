 ...
 */
expect class Libp2pTransport(config: Libp2pConfig) : Transport
...
data class Libp2pConfig(
    ...
    val protocolPrefix: String = "/aura",
    ...
    // Phase-2 efficiency / reachable-tier toggles (applied on the Go host before creation).
    // Enable a limited circuit-relay-v2 SERVICE on this node (only meaningful when reachable).
    val relayServiceEnabled: Boolean = false,
    // AutoRelay bound to a fixed TRUSTED relay set (comma-separated multiaddrs of the app's own
    // reachable nodes), NOT the open DHT. Empty = AutoRelay off (preserves the no-open-AutoRelay stance).
    val autoRelayStaticRelays: String = "",
    // GossipSub peer scoring (StrictSign stays on regardless).
    val gossipHardeningEnabled: Boolean = false,
) {
    override fun equals(other: Any?): Boolean {
        ...
        return identityKey.contentEquals(other.identityKey) &&
            ...
            relayServiceEnabled == other.relayServiceEnabled &&
            autoRelayStaticRelays == other.autoRelayStaticRelays &&
            gossipHardeningEnabled == other.gossipHardeningEnabled
    }
    ...
    override fun hashCode(): Int {
        ...
        h = 31 * h + relayServiceEnabled.hashCode()
        h = 31 * h + autoRelayStaticRelays.hashCode()
        h = 31 * h + gossipHardeningEnabled.hashCode()
        return h
    }
}
